...
public class OnlineBord : MonoBehaviourPunCallbacks
{
    ...
    void MapCreate()  //盤面生成
    {
        for (int i = 0; i < Chessbord.GetLength(0); i++)
        {
            for (int j = 0; j < Chessbord.GetLength(1); j++)
            {
                if (Chessbord[i, j] == 0)
                {
                   PhotonNetwork.Instantiate("Bord_B", new Vector3(i + 0.5f, -0.5f, j + 0.5f), Quaternion.identity);
                    Bord_B.name = i + "-" + j.ToString();
                }
                else /*if (Chessbord[i, j] == 1)*/
                {
                   PhotonNetwork.Instantiate("Bord_W", new Vector3(i + 0.5f, -0.5f, j + 0.5f), Quaternion.identity);
