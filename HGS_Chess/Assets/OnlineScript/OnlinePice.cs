...
public class OnlinePice: MonoBehaviourPunCallbacks
{
    ...
    //レイを作成しコライダーに当たったら色を変える==============================================================================
       ...
       void MoveChess(int x, int y)
        ...
        //=========================================================================================
        //※駒の生成===============================================================================
        void PiceCreat(int index, int x, int y)
        {
        //if (Player_Num == 1 )//黒駒 
        //{
          if (PhotonNetwork.PlayerList.Length == 1/*|| index < 2*/)
          {
            if (index < 2)//このif文を入れないと黒駒がすべて180度回転し反対方向を向く
            {
                GameObject go = PhotonNetwork.Instantiate(chessmPrefabs[index].name, GetTileCenter(x, y), Quaternion.identity, 0) as GameObject;
                Debug.Log("できた");
        ...
        {
            GameObject go = PhotonNetwork.Instantiate(chessmPrefabs[index].name, GetTileCenter(x, y), orientation, 0) as GameObject;
            Debug.Log("できた!!");
            go.transform.SetParent(transform);
            moves[x, y] = go.GetComponent<Move>();
            moves[x, y].SetPosition(x, y);
